...
bool comparaAgendamentos(ListaLog *S, ListaLog *S_linha){
	...
	for (int i = 0; i < tam_array; i++){
		if (ultimaEscrita(S, array_atributos[i]) != ultimaEscrita(S_linha, array_atributos[i])){
			printf("ultima escrita em %c diferente \n", array_atributos[i]);
			return false;	
		}
	}
	...
	while (iterator != NULL){
		...
		if (iterator -> operacao == 'R'){
			...
			if (escritaAnterior(aux) != escritaAnterior(iterator)){
				printf("deu ruim: %c em %d tem W em %d quando deveria ter %d \n", iterator -> atributo, iterator -> id, escritaAnterior(aux), escritaAnterior(iterator));
				return false;	
			}
		}
...
int generate(int size, int n, int *A, ListaLog *S){
	...
	if (1 == n){
		...
		bool equivalente = comparaAgendamentos(S, S_linha);
		if (equivalente == true)
			printf("Equivalem\n");
		else
			printf("Nao equivalem\n");
	}
    else {
		for (int i = 0; i < n  ; i++){
			generate(size, n - 1, A, S);
			if (n % 2 == 0){
				troca(&A[i], &A[n-1]);
			}
			else {
...
bool equivalente(VertexList *vertices, ListaLog *logs){
	...
	printArray(tam_vertices, arrayVertices);
	generate(tam_vertices, tam_vertices, arrayVertices, logs);
